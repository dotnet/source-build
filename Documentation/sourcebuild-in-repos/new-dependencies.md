...
.NET dependencies are from a Microsoft .NET project.  Adding a new .NET dependency is usually quite easy, especially if the repo uses Arcade to build.  Internal dependencies are from Microsoft repos but not .NET.  Adding a new internal dependency is usually pretty straightforward but does have some edge cases.  Use this checklist:
