...
Pre-built detection identifies various types of dependencies. These include direct dependencies, indirect or transitive dependencies, and build tooling. Additionally, pre-built detection also considers dependencies retrieved from external sources that are not explicitly excluded from the detection process.
...
> [!NOTE]
>
> Many repositories in the source-build graph use NuGet's Central Package Management feature which can help with lifting and unifying transitive dependencies via the transitive pinning feature. If you're experiencing issues with a transitive dependency, look into using [CPM](https://learn.microsoft.com/en-us/nuget/consume-packages/central-package-management#transitive-pinning).
...
> [!NOTE] 
>
> The pre-built report that is generated by pre-built detection can be found at `./artifacts/sb/prebuilt-report`. This directory contains the following files:
...
1. Decide which pre-builts to tackle.
    1. If you have detected a new pre-built that is not in the current baseline, `./eng/SourceBuildPrebuiltBaseline.xml`, continue to step 3.
    2. If you are trying to eliminate a pre-built that currently exists in the baseline, do so cautiously. More often than not, pre-builts in the baseline contain comments about why they are in the baseline and when/if it is appropriate to eliminate the pre-built.
...
At this point, determine how to handle the dependency by following the [adding dependencies documentation](https://github.com/dotnet/source-build/blob/main/Documentation/sourcebuild-in-repos/new-dependencies.md#adding-dependencies).
...
At this point, determine how to handle the dependency by following [adding dependencies documentation](https://github.com/dotnet/source-build/blob/main/Documentation/sourcebuild-in-repos/new-dependencies.md#adding-dependencies).
...
At this point, determine how to handle the dependencies by following [adding dependencies documentation](https://github.com/dotnet/source-build/blob/main/Documentation/sourcebuild-in-repos/new-dependencies.md#adding-dependencies).
...
With this ignore pattern in place, pre-built detection will not mark the Foo.Bar.x.y.x as a pre-built.
...
The new file can be found at `./artifacts/sb/prebuild-report/generated-new-baseline.xml`.
