        ...
        private async Task<bool> ExecuteAsync()
        {
            ...
            while(true)
            {
                ...
                catch (Exception e)
                {
                    ...
                    if (canRetry)
                    {
                        Log.LogWarningWithCodeFromResources("DownloadFileSB.Retrying", SourceUrl, retryAttemptCount + 1, RetryDelayMilliseconds, actualException.Message);
                    ...
                    {
                        Log.LogErrorWithCodeFromResources("DownloadFileSB.ErrorDownloading", SourceUrl, actualException.Message);
                        break;
                    }
                }
            }
        ...
        private async Task DownloadAsync(Uri uri, CancellationToken cancellationToken)
        {
            ...
            using (var client = new HttpClient(HttpMessageHandler ?? new HttpClientHandler(), disposeHandler: true))
            {
                ...
                using (HttpResponseMessage response = await client.GetAsync(uri, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    {
                        Log.LogErrorWithCodeFromResources("DownloadFileSB.ErrorUnknownFileName", SourceUrl, nameof(DestinationFileName));
                        return;
                    }
