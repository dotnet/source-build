...
function installdotnetcli()
{
    ...
    if [[ ! -d "$DOTNET_INSTALL_DIR" ]]; then 
        ...
        curl -sSL "$DOTNET_INSTALL_SCRIPT_URL" | bash /dev/stdin --install-dir $DOTNET_INSTALL_DIR --version $DOTNET_CLI_VERSION --verbose
    fi
    
    echo "installdotnetcli: Complete. Elapsed $(( SECONDS - start ))"
}

function setdotnetpath()
{
    [[ -z "$DOTNET_PATH" ]] && DOTNET_PATH="$TP_TOOLS_DIR/dotnet/dotnet"
