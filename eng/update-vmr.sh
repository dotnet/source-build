...
print-help ()
{
   ...
   echo "--upstream, -u        A valid git URL to the upstream repo to base the update branch off of"
   echo "--tarball, -t         The tarball to update the VMR with"
   echo "--isDryRun, -d        If set, then don't push results to AzDo"
   echo "--help, -h            (Optional) print this help message and exit"
   echo
}

SHORT=r:v:u:t:h
LONG=releaseChannel:,sdkVersion:,upstream:,tarball:,help
